...
void main(void) {
	...
	float t= time*0.25;
	vec2 pos = uv * vec2(1.0, 0.1 ) - vec2(t * 0.05, t * 0.5);
	
    float re = pow ( 16.0 , ( 0.9 ) ) ;
	...
	rgb += mix(vec3(1.0, 0.6 , pow( 3.0 , 2.0 ) )*fade, vec3(0.0), smoothEdge(0.03, 0.5, d)) * 0.5;
	rgb += mix(vec3(0.1, cos ( time ) * 0.7 , 0.0)*fade, vec3(0.0), smoothEdge(0.03, 1.0, d)) * ( 0.15 + pow ( 1.0 , 3.0 )) ;
	
	rgb = mix(rgb, vec3(0.2 + + sin( time ) * 0.6 ,0.6 + sin( time ) * .2 ,0.99), tan( time ) * dir.y );
